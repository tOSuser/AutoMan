#
# TestDataTransformer.py
#
# Description :
# --------------------------------------
# Data Transformer unittests
#
#
...
from transformers.DataTransformer import DataTransformer

# TestDataTransformer : Unit tests
#---------------------------------------
class TestDataTransformer(TestCase,TestHelper):
	"""
	Text/Data Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myDataTransformer= DataTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myDataTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myDataTransformer.runTagAction('.txt',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myDataTransformer.runTagAction('.txt',{'src' : 'source_file','dest' : 'destination_file'}),False)
