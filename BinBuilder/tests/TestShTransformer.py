#
# TestShTransformer.py
#
# Description :
# --------------------------------------
# Sh Transformer unittests
#
#
...
from transformers.ShTransformer import ShTransformer

# TestShTransformer : Unit tests
#---------------------------------------
class TestShTransformer(TestCase,TestHelper):
	"""
	sh/bash Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myShTransformer= ShTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myShTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myShTransformer.runTagAction('.sh',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myShTransformer.runTagAction('.sh',{'src' : 'source_file','dest' : 'destination_file'}),False)
