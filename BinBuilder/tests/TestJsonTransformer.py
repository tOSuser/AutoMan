#
# TestJsonTransformer.py
#
# Description :
# --------------------------------------
# Json Transformer unittests
#
#
...
from transformers.JsonTransformer import JsonTransformer

# TestJsonTransformer : Unit tests
#---------------------------------------
class TestJsonTransformer(TestCase,TestHelper):
	"""
	Json Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myJsonTransformer= JsonTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myJsonTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myJsonTransformer.runTagAction('.json',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myJsonTransformer.runTagAction('.json',{'src' : 'source_file','dest' : 'destination_file'}),False)
