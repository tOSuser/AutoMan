#
# TestXmlTransformer.py
#
# Description :
# --------------------------------------
# Xml Transformer unittests
#
#
...
from transformers.XmlTransformer import XmlTransformer

# TestXmlTransformer : Unit tests
#---------------------------------------
class TestXmlTransformer(TestCase,TestHelper):
	"""
	Xml Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myXmlTransformer= XmlTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myXmlTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myXmlTransformer.runTagAction('.xml',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myXmlTransformer.runTagAction('.xml',{'src' : 'source_file','dest' : 'destination_file'}),False)
