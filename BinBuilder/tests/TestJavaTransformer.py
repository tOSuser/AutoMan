#
# TestJavaTransformer.py
#
# Description :
# --------------------------------------
# Java Transformer unittests
#
#
...
from transformers.JavaTransformer import JavaTransformer

# TestJavaTransformer : Unit tests
#---------------------------------------
class TestJavaTransformer(TestCase,TestHelper):
	"""
	Java Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myJavaTransformer= JavaTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myJavaTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myJavaTransformer.runTagAction('.java',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myJavaTransformer.runTagAction('.java',{'src' : 'source_file','dest' : 'destination_file'}),False)
