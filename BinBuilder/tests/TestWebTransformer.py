#
# TestWebTransformer.py
#
# Description :
# --------------------------------------
# Web Transformer unittests
#
#
...
from transformers.WebTransformer import WebTransformer

# WebJavaTransformer : Unit tests
#---------------------------------------
class TestWebTransformer(TestCase,TestHelper):
	"""
	Web (html, css, ico, png, svg) Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myWebTransformer= WebTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myWebTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myWebTransformer.runTagAction('.html',{'src' : 'source_file','dest' : 'destination_file'}),True)
		self.assertEqual(self.myWebTransformer.runTagAction('.css',{'src' : 'source_file','dest' : 'destination_file'}),True)
		self.assertEqual(self.myWebTransformer.runTagAction('.ico',{'src' : 'source_file','dest' : 'destination_file'}),True)
		self.assertEqual(self.myWebTransformer.runTagAction('.png',{'src' : 'source_file','dest' : 'destination_file'}),True)
		self.assertEqual(self.myWebTransformer.runTagAction('.svg',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myWebTransformer.runTagAction('.html',{'src' : 'source_file','dest' : 'destination_file'}),False)
