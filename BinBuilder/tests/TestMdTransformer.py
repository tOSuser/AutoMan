#
# TestMdTransformer.py
#
# Description :
# --------------------------------------
# Md Transformer unittests
#
#
...
from transformers.MdTransformer import MdTransformer

# TestMdTransformer : Unit tests
#---------------------------------------
class TestMdTransformer(TestCase,TestHelper):
	"""
	Markdown Transformer unittest.
		-
		-
	"""
	"""
	...
	def setUp(self):
		self.INFO("\n[RUN TEST]",False)
		self.myMdTransformer= MdTransformer()
	...
	def test_runTagAction(self,mock_copyfile):
		...
		self.assertEqual(self.myMdTransformer.runTagAction('invalidtag',{'src' : 'source_file','dest' : 'destination_file'}),False)
		self.assertEqual(self.myMdTransformer.runTagAction('.md',{'src' : 'source_file','dest' : 'destination_file'}),True)

		myfakeerror_copyfile = FakeError("copy error")
		mock_copyfile.side_effect = myfakeerror_copyfile.genrateError
		self.assertEqual(self.myMdTransformer.runTagAction('.md',{'src' : 'source_file','dest' : 'destination_file'}),False)
